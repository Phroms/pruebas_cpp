...
Channel::Channel(std::string name, std::string key, Client &admin)
{
	...
	modes['i'] = false;  // Canal solo por invitación
    modes['t'] = false;  // Solo operadores pueden cambiar el tópico
    modes['l'] = false;  // Límite de usuarios
    modes['k'] = false;  // Canal protegido por contraseña
